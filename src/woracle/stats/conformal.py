...
threshold λ admits more rollouts but raises the risk of admitting BROKEN
ones. Given a labeled calibration set, ``calibrate_threshold`` returns the
most permissive λ whose finite-sample-corrected broken-admission risk stays
≤ α — the conformal-risk-control bound (Angelopoulos et al., "Conformal Risk
Control": choose λ̂ = inf{λ : (n/(n+1))·R̂(λ) + B/(n+1) ≤ α} for a bounded
...
def calibrate_threshold(
    ...
    *,
    ...
    Risk(λ) = P(broken admitted) = mean_{broken}[ value >= λ ] — monotone
    non-increasing in λ, bounded in [0,1] ⇒ CRC applies with B = 1:
        λ̂ = inf{ λ : (n_b/(n_b+1))·R̂(λ) + 1/(n_b+1) ≤ alpha }
    computed over the candidate λ grid of observed values (+∞ fallback).
    """
    ...
    # Candidate thresholds: descending unique values; pick the smallest λ
    ...
    for lam in candidates:
        ...
        if bound <= alpha:
            ...
            break  # risk is monotone: lower λ only admits more broken rollouts
    if chosen is None:
